...
#ref_df = ref_df[ref_df['engagementType']=='tweet']
...
def generate_segments(users_dict,method_name,threshold):
    ...
    for key,users in users_dict.items():
        ...
        if("hash" in method_name and key == '1'):
            ...
            for tweet in filtered_df['contentText'].values.astype(str):
                ...
                tags = list(set(filter(lambda x:x!=" " and len(x)>=3,tags)))
                if(len(tags) >=3):
    ...
    json_file = open(os.path.join(JSON_DIR,"segments_{METHOD}_THRESH_{THRESHOLD}.json".format(METHOD=method_name,THRESHOLD=str(threshold))), "w")  
    json.dump(json_to_be_saved, json_file, indent = 6)  
    json_file.close()  
    
    del json_to_be_saved
    
def generate_messages(users_dict,method_name,threshold):
    ...
    json_file = open(os.path.join(JSON_DIR,"message_groups_{METHOD}_THRESH_{THRESHOLD}.json".format(METHOD=method_name,THRESHOLD=str(threshold))), "w")  
    
    json.dump(json_to_be_saved, json_file, indent = 6)  
    json_file.close()  

def retrieve_users(graphs_dir,threshold):
    ...
    for graph_dir in graphs_dir:
        ...
        # Generate_JSONs
        #generate_segments({'1':coordinated_users,'0':non_coordinated_users},method_name,threshold)
        generate_messages({'1':coordinated_users,'0':non_coordinated_users},method_name,threshold)
        
        # Test warning
        warnings.warn("Completed for {METHOD}".format(METHOD=method_name))
        
ROOT_DIR = "/scratch1/ashwinba/cache/INCAS/phase_2"
graphs_dir = glob.glob(os.path.join(ROOT_DIR,"*.gexf"))
#graphs_dir = [os.path.join(ROOT_DIR,"hashSeq_INCAS_TA2.gexf")]
graphs_dir = [os.path.join(ROOT_DIR,"coURL_INCAS_TA2_1.gexf")]
#graphs_dir = ["/scratch1/ashwinba/cache/INCAS/phase_2/textsim_INCAS_T2.gexf"]
#graphs_dir = [os.path.join(ROOT_DIR,"coRetweet_INCAS_TA2.gexf")]
warnings.warn(str(graphs_dir))

#filter_graphs(graphs_dir=graphs_dir,threshold=90)
#retrieve_users(graphs_dir,threshold=95)
retrieve_users(graphs_dir,threshold=90)